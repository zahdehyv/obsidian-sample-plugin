export class AudioRecorder {
    ...
    render(container: HTMLDivElement) {
        this.icon = container.createEl('span', {
            attr: { style: 'cursor: pointer; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; border-radius: 50%; background: transparent; transition: background 0.2s;' }
        });
        this.icon.innerHTML = `
    ...
    private async startRecording() {
        ...
        this.mediaRecorder.onstop = () => {
            ...
            this.addMessage('', '', audioUrl);
            this.setRecordingIndicator(false); // Reset background when recording stops
        };

        this.mediaRecorder.start();
        this.setRecordingIndicator(true); // Set red background when recording starts
    }
